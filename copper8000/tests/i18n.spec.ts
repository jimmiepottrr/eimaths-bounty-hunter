...
test('แอดมิน: เพิ่มภาษาใหม่ / ภาษาหลักไม่มีปุ่มลบ / ปิดภาษาแล้วหายจาก picker', async ({ page }) => {
  ...
  // ภาษาใหม่โผล่ใน picker และใช้งานได้จริง
  await page.locator('.lang-picker select').selectOption('ja');
  await page.goto('/#/');
  await expect(page.getByRole('heading', { name: '本日の金属買取価格' })).toBeVisible();

  // กลับไปปิดภาษาจีน → หายจาก picker
  await page.locator('.lang-picker select').selectOption('th');
  await page.goto('/#/admin');
  await page.getByRole('button', { name: 'ภาษา', exact: true }).click();
  ...
  await expect(page.locator('.lang-picker select option[value="zh"]')).toHaveCount(0);
